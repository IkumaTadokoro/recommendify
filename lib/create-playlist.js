...
const createPlaylist = async () => {
  if (!accessToken && !refreshToken) {
    console.log('Access token is not retrived. You should get access token via \'recommendify settoken\' command.')
    process.exit(1)
  }

  spotifyApi.setAccessToken(accessToken)

  const description = `This playlist is created at ${formattedToday} and created by Recommendify.`
  ...
  const response = await prompt([
    {
      type: 'multiselect',
      name: 'songs',
      message: 'Pick your favorite songs upto 3 from You have listened recently. Press Space key to select songs and Enter key to confirm.',
      limit: 10,
    ...
    },
    {
      type: 'scale',
      name: 'valence',
      message: 'Choose positiveness of songs（高い方がよりポジティブ、元気が出る曲）. Press Arrow keys to choose answer and Enter key to confirm.',
      scale: [
    ...
    },
    {
      type: 'scale',
      name: 'popularity',
      message: 'Choose popularity of artists（高い方が人気のあるアーティスト）. Press Arrow keys to choose answer and Enter key to confirm.',
      scale: [
    ...
    },
    {
      type: 'scale',
      name: 'tempo',
      message: 'Choose BPM of songs. Press Arrow keys to choose answer and Enter key to confirm.',
      scale: [
        { name: '75', message: '' },
        { name: '100', message: '' },
        { name: '125', message: '' },
